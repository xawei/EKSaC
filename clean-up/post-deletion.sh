...
EKS_CLUSTER=$(timeout 60 aws eks describe-cluster --region $REGION --name $CLUSTER_NAME --query 'cluster.name' --output text 2>/dev/null || echo "")

if [ $? -eq 124 ]; then
  echo "WARNING: AWS CLI command timed out. Skipping EKS cluster check."
  EKS_CLUSTER=""
elif [ ! -z "$EKS_CLUSTER" ] && [ "$EKS_CLUSTER" != "None" ] && [ "$EKS_CLUSTER" != "null" ]; then
  echo "Found EKS cluster: $EKS_CLUSTER"
  
  # Delete node groups first
  echo "Checking for node groups..."
  NODE_GROUPS=$(timeout 60 aws eks list-nodegroups --region $REGION --cluster-name $CLUSTER_NAME --query 'nodegroups' --output text 2>/dev/null || echo "")
  
  if [ $? -eq 124 ]; then
    echo "WARNING: Node groups check timed out."
  elif [ ! -z "$NODE_GROUPS" ] && [ "$NODE_GROUPS" != "None" ]; then
    ...
    for ng in $NODE_GROUPS; do
      timeout 600 aws eks wait nodegroup-deleted --region $REGION --cluster-name $CLUSTER_NAME --nodegroup-name $ng || echo "Timeout waiting for $ng deletion"
  ...
  FARGATE_PROFILES=$(timeout 60 aws eks list-fargate-profiles --region $REGION --cluster-name $CLUSTER_NAME --query 'fargateProfileNames' --output text 2>/dev/null || echo "")
  
  if [ $? -eq 124 ]; then
    echo "WARNING: Fargate profiles check timed out."
  elif [ ! -z "$FARGATE_PROFILES" ] && [ "$FARGATE_PROFILES" != "None" ]; then
    ...
    for fp in $FARGATE_PROFILES; do
      timeout 600 aws eks wait fargate-profile-deleted --region $REGION --cluster-name $CLUSTER_NAME --fargate-profile-name $fp || echo "Timeout waiting for $fp deletion"
  ...
  ADDONS=$(timeout 60 aws eks list-addons --region $REGION --cluster-name $CLUSTER_NAME --query 'addons' --output text 2>/dev/null || echo "")
  
  if [ $? -eq 124 ]; then
    echo "WARNING: Addons check timed out."
  elif [ ! -z "$ADDONS" ] && [ "$ADDONS" != "None" ]; then
  ...
  timeout 1200 aws eks wait cluster-deleted --region $REGION --name $CLUSTER_NAME || echo "Timeout waiting for cluster deletion"
  echo "EKS cluster $CLUSTER_NAME deletion process completed."
else
  echo "No EKS cluster found with name: $CLUSTER_NAME (or check timed out)"
fi
...
ORPHANED_INSTANCES=$(timeout 60 aws ec2 describe-instances \
  --region $REGION \
...
if [ $? -eq 124 ]; then
  echo "WARNING: EC2 instances check timed out."
elif [ ! -z "$ORPHANED_INSTANCES" ]; then
  ...
  timeout 300 aws ec2 wait instance-terminated --region $REGION --instance-ids $ORPHANED_INSTANCES || echo "Timeout waiting for instance termination"
...
ORPHANED_VOLUMES=$(timeout 60 aws ec2 describe-volumes \
  --region $REGION \
...
if [ $? -eq 124 ]; then
  echo "WARNING: EBS volumes check timed out."
elif [ ! -z "$ORPHANED_VOLUMES" ]; then
...
ORPHANED_EFS=$(timeout 60 aws efs describe-file-systems \
  --region $REGION \
  --query "FileSystems[?Tags[?Key=='kubernetes.io/cluster/$CLUSTER_NAME' && Value=='owned']].FileSystemId" \
  --output text 2>/dev/null || echo "")

if [ $? -eq 124 ]; then
  echo "WARNING: EFS filesystems check timed out."
elif [ ! -z "$ORPHANED_EFS" ]; then
...
ORPHANED_S3_BUCKETS=$(timeout 60 aws s3api list-buckets \
  --query "Buckets[?contains(Name, '$CLUSTER_NAME')].Name" \
  --output text 2>/dev/null || echo "")

if [ $? -eq 124 ]; then
  echo "WARNING: S3 buckets check timed out."
else
...
ORPHANED_SGS=$(timeout 60 aws ec2 describe-security-groups \
  --region $REGION \
  --filters "Name=tag:kubernetes.io/cluster/$CLUSTER_NAME,Values=owned" \
  --query 'SecurityGroups[?GroupName!=`default`].GroupId' --output text 2>/dev/null || echo "")

if [ $? -eq 124 ]; then
  echo "WARNING: Security groups check timed out."
elif [ ! -z "$ORPHANED_SGS" ]; then
